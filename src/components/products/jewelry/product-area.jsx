...
const ProductArea = ({ defaultCategory = 'All Collection' }) => {
      ...
      <>
        <div className="row align-items-end mb-4">
          <div className="col-lg-6">
            <div className="easy-shoping-product-area__header">
              <span className="easy-shoping-product-area__label">{subtitle}</span>
              <h2 className="easy-shoping-product-area__title">{title}</h2>
            </div>
          </div>
          <div className="col-lg-6">
            <div className="easy-shoping-product-area__tabs-wrap">
              <nav className="easy-shoping-product-area__tabs" role="tablist">
                {tabs.map((tab, i) => (
                  <button
                    key={i}
                    ref={activeTab === tab ? activeRef : null}
                    onClick={(e) => handleActiveTab(e, tab)}
                    className={`easy-shoping-product-area__tab ${activeTab === tab ? "active" : ""}`}
                  >
                    {tab.split("-").join(" ")}
                    <span className="easy-shoping-product-area__tab-count">{getCategoryCount(tab)}</span>
                  </button>
                ))}
                <span ref={marker} className="easy-shoping-product-area__tab-marker" aria-hidden="true" />
              </nav>
            </div>
          </div>
        </div>
        ...
        <div className="text-center mt-5">
          <button type="button" onClick={handleShowMore} className="easy-shoping-product-area__show-more">
            Show more
            <span className="easy-shoping-product-area__show-more-arrow"><ArrowRight /></span>
          </button>
        </div>
      </>
    );
  ...
  return (
    <section className="easy-shoping-product-area">
